...
type AdStatus = "accepted" | "rejected" | "offered" | null;
...
const initialBuyerAds: BuyerAd[] = [
  ...
  { buyerRef: "B-3387", buyerType: "Café / Catering", region: "Howrah district", product: "Milk", qty: "40 L", price: 48, status: null, bargainOpen: false, offer: "" },
  { buyerRef: "B-9021", buyerType: "General Store", region: "Kolkata region", product: "Brinjal", qty: "75 kg", price: 27, status: null, bargainOpen: false, offer: "" },
  { buyerRef: "B-6640", buyerType: "Wholesale Trader", region: "West Bengal (state-wide)", product: "Wheat", qty: "500 kg", price: 21, status: null, bargainOpen: false, offer: "" },
];
...
function buildNotifications(buyerAds: BuyerAd[], shipmentList: Shipment[]): NotificationItem[] {
  ...
  buyerAds.forEach((ad) => {
    if (ad.status === "accepted") {
      ...
      items.push({ kind: "offer", title: `Counter-offer sent to ${ad.buyerRef}`, sub: `${ad.product} · awaiting response at ₹${ad.offer}` });
    }
  });
          ...
          <div className="khl-buyer-ad" key={i}>
            ...
            {!ad.status && (
              <div className="khl-ad-actions">
                ...
                <button className="khl-ad-btn reject" onClick={() => respondAd(i, "rejected")}>Reject</button>
                <button className="khl-ad-btn bargain" onClick={() => toggleBargain(i)}>Bargain</button>
              </div>
            )}

            {!ad.status && ad.bargainOpen && (
              <div className="khl-bargain-row">
                <input type="number" placeholder="Counter price ₹" value={ad.offer} onChange={(e) => setOfferValue(i, e.target.value)} />
                <button onClick={() => sendOffer(i)}>Send Offer</button>
              </div>
            )}

            {ad.status === "accepted" && <span className="khl-status-tag accepted">✓ Accepted</span>}
            {ad.status === "rejected" && <span className="khl-status-tag rejected">✕ Rejected</span>}
            {ad.status === "offered" && <span className="khl-status-tag offered">↔ Counter-offer sent: ₹{ad.offer} — awaiting buyer response</span>}
          </div>
        ))}
      </div>
    </div>
...
export default function KhetLinkDashboard() {
  const [activeFullScreen, setActiveFullScreen] = useState<FullScreenId>(null);
  ...
  const [profileName] = useState("Ramesh Kumar");

  const offersRef = useRef<HTMLDivElement>(null);

  const openFullScreen = (id: FullScreenId) => setActiveFullScreen(id);
  const closeFullScreen = () => setActiveFullScreen(null);
  ...
  const respondAd = (idx: number, status: AdStatus) => setBuyerAds((prev) => prev.map((a, i) => (i === idx ? { ...a, status, bargainOpen: false } : a)));
  const toggleBargain = (idx: number) => setBuyerAds((prev) => prev.map((a, i) => (i === idx ? { ...a, bargainOpen: !a.bargainOpen } : a)));
  const setOfferValue = (idx: number, val: string) => setBuyerAds((prev) => prev.map((a, i) => (i === idx ? { ...a, offer: val } : a)));
  const sendOffer = (idx: number) =>
    setBuyerAds((prev) => prev.map((a, i) => (i === idx && a.offer ? { ...a, status: "offered", bargainOpen: false } : a)));
